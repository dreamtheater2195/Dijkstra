...
namespace Dijkstra.Classes
{
    public class Node:IComparable<Node>
    {
        ...
        public Node(Point location, Point center, string label, double diameter) : this(location, label, diameter)
        {
            this._center = center;
        }
        ...
        {
            double distance = Math.Sqrt(Math.Pow(p.X - _center.X, 2) + Math.Pow(p.Y - _center.Y, 2));
            return distance <= (_diameter / 2);
        }
